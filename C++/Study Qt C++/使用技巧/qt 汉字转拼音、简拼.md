# qt 汉字转拼音、简拼
![在这里插入图片描述](https://img-blog.csdnimg.cn/2020030714510659.gif)
汉字转拼音控件作者  [作者:feiyangqingyun(QQ:517216493)](https://gitee.com/feiyangqingyun)
下载地址: [https://pan.baidu.com/s/1SW_81M-Rzdav1EEW5ZgjEg](https://pan.baidu.com/s/1SW_81M-Rzdav1EEW5ZgjEg) 提取码: 6h2c
...

{
  ...
  public:
    ...
    explicit ZhToPY(QObject *parent = nullptr);
...
QString ZhToPY::zhToJP(const QString &chinese) {
    ...
    for(int i = 0; i < chinese.length(); i++) {
        ...
        if((vChar >= '0' && vChar <= '9')) {
            ...
            index = static_cast<qint32>(vChar - 19968);
