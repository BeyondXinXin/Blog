...
int main(int argc, char * argv[])
{
    ...
    wid->setFixedSize(400, 800);

    // 几个按钮以及逻辑
    GenerateTestButton(wid);

    vtkNew<vtkImagePlaneWidget> planeWidget;
    planeWidget->SetInteractor(wid->renderWindow()->GetInteractor());
    ...
    vtkNew<vtkRenderer> rendere_1;
    rendere_1->AddActor(planeActor);
    wid->setFixedSize(800, 600);

    wid->renderWindow()->AddRenderer(renderer);
    wid->renderWindow()->AddRenderer(rendere_1);

    renderer->SetViewport(0, 0, .5, 1);
    rendere_1->SetViewport(0.5, 0, 1, 1);
    // 显示
    wid->show();
    planeWidget->On();
